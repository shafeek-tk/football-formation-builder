...
test.describe('Football Formation Builder', () => {
  ...
  test('should edit player names', async ({ page }) => {
    ...
    // Click on first player name to edit (refactored uses contentEditable)
    ...
    // Type new name
    await firstPlayerName.fill('EDITED NAME');
    ...
    const newName = await firstPlayerName.textContent();
    expect(newName).toBe('EDITED NAME');
  });
  ...
  test('should generate shareable URL', async ({ page }) => {
    ...
    // Override shareURL function in formationBuilder
    await page.evaluate(() => {
      return new Promise((resolve) => {
        const checkBuilder = () => {
          if (window.formationBuilder) {
            window.capturedShareUrl = '';
            const originalShareURL = window.formationBuilder.shareURL.bind(window.formationBuilder);
            window.formationBuilder.shareURL = function(url) {
              window.capturedShareUrl = url;
              // Mock clipboard to prevent blocking
  ...
  test('should preserve names when changing formations', async ({ page }) => {
    ...
    // Edit multiple home team player names using evaluate
    const homeNames = ['HOME A', 'HOME B', 'HOME C'];
    for (let i = 0; i < homeNames.length; i++) {
      await page.locator('.my-team .player-name').nth(i).evaluate((el, name) => {
        el.textContent = name;
      }, homeNames[i]);
    }
    
    // Edit multiple away team player names using evaluate
    const awayNames = ['AWAY X', 'AWAY Y', 'AWAY Z'];
    for (let i = 0; i < awayNames.length; i++) {
      await page.locator('.opp-team .player-name').nth(i).evaluate((el, name) => {
        el.textContent = name;
      }, awayNames[i]);
    }
  ...
  test('should load 7v7 format and display formation controls', async ({ page }) => {
    ...
    await expect(page.locator('.mode-toggle .mode-btn', { hasText: '11v11' })).toBeVisible();
    await expect(page.locator('.mode-toggle .mode-btn', { hasText: '7v7' })).toBeVisible();
    await expect(page.locator('.mode-toggle .mode-btn', { hasText: '6v6' })).toBeVisible();
  });
  ...
  test('should preserve names when changing 7v7 formations', async ({ page }) => {
    ...
    for (let i = 0; i < homeNames.length; i++) {
      ...
      await player.evaluate((el, name) => { el.textContent = name; }, homeNames[i]); await page.waitForTimeout(200);
      await expect(player).toHaveText(homeNames[i]);
    }
    ...
    for (let i = 0; i < awayNames.length; i++) {
      ...
      await player.evaluate((el, name) => { el.textContent = name; }, awayNames[i]); await page.waitForTimeout(200);
      await expect(player).toHaveText(awayNames[i]);
    }
  ...
  test('should work on mobile viewport for 7v7', async ({ page }) => {
    ...
    const modeButton = page.locator('.mode-toggle .mode-btn', { hasText: '7v7' });
    await expect(modeButton).toBeVisible();
  ...
  test('should edit 7v7 player names', async ({ page }) => {
    ...
    // Edit name directly
    await firstPlayerName.evaluate((el, name) => { el.textContent = name; }, 'EDITED 7V7');
    await page.waitForTimeout(200);
  ...
  test('should generate shareable URL for 7v7', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.capturedShareUrl = '';
      const originalShareURL = window.formationBuilder.shareURL;
      window.formationBuilder.shareURL = function(url) {
        window.capturedShareUrl = url;
        if (navigator.clipboard) {
          navigator.clipboard.writeText(url).catch(() => {});
  ...
  test('should work on mobile viewport for 6v6', async ({ page }) => {
    ...
    // Mode buttons should be visible on mobile (use button selector to avoid duplicate)
    const modeButton = page.locator('.mode-toggle .mode-btn', { hasText: '6v6' });
    await expect(modeButton).toBeVisible();
  ...
  test('should edit 6v6 player names', async ({ page }) => {
    ...
    // Edit name directly
    await firstPlayerName.evaluate((el, name) => { el.textContent = name; }, 'EDITED 6V6');
    await page.waitForTimeout(200);
